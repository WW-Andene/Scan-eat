...
fun mapOffProduct(raw: OffProductRaw): Product? {
    ...
    val nutrition = NutritionPer100g(
        ...
        // even though a salt value is derivable from data already fetched.
        saltG         = numOrNull(nm["salt_100g"]) ?: (numOrNull(nm["sodium_100g"])?.times(SODIUM_TO_SALT_FACTOR) ?: 0.0),
...
fun detectSourceConflicts(off: Product, llm: Product): List<SourceConflict> {
    ...
    // user-visible and most error-prone field (mapOffProduct even derives it
