...
// kidney_disease/pregnancy — see its own file) and the hint panel, which
// previously ignored Profile.healthConditions entirely. Same "trace back
// to a concrete sourced field" discipline as the rest of this package:
// every entry here matches an actual ingredient-list keyword and cites the
// public-health body that publishes the guidance.
//
// Deliberately narrow: pregnancy-related food-safety guidance (ANSES —
// the French food safety agency's own public pregnancy dietary advice,
// mirrored by most EU national equivalents) is well-documented, stable,
// and non-controversial. Not attempting condition x ingredient matrices
// where no comparably authoritative, stable public guidance exists (e.g.
// "digestive_disorders" is too heterogeneous a bucket to map to specific
// ingredients without guessing).
// ============================================================================

private data class PregnancyGuidance(
...
private val PREGNANCY_GUIDANCE: List<PregnancyGuidance> = listOf(
    PregnancyGuidance(listOf("tartare", "carpaccio", "viande crue", "steak tartare", "raw meat", "rare beef"),
    ...
    PregnancyGuidance(listOf("sushi", "sashimi", "saumon fumé", "smoked salmon", "poisson cru", "raw fish", "ceviche"),
    ...
    PregnancyGuidance(listOf("thon", "espadon", "requin", "lamproie", "tuna", "swordfish", "shark", "marlin"),
    ...
    PregnancyGuidance(listOf("lait cru", "fromage au lait cru", "fromage à pâte molle", "raw milk", "unpasteurized", "soft cheese"),
    ...
    PregnancyGuidance(listOf("foie", "pâté", "liver", "pate", "foie gras"),
    ...
    PregnancyGuidance(listOf("jambon cru", "charcuterie", "salami", "chorizo", "cured meat", "deli meat", "prosciutto"),
    ...
    PregnancyGuidance(listOf("germes crus", "pousses crues", "graines germées", "raw sprouts", "alfalfa sprouts"),
 ...
 * Currently only "pregnancy" has a mapped dictionary — see this file's header
 * for why the other free-form condition keys aren't guessed at.
 */
fun findHealthConditionGuidance(ingredients: List<Ingredient>, healthConditions: Set<String>, lang: String): List<String> {
    if ("pregnancy" !in healthConditions) return emptyList()
    val en = lang == "en"
    return matchIngredientDictionary(ingredients, PREGNANCY_GUIDANCE, PregnancyGuidance::names)
