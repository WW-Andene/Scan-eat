...
internal fun PermissionBanner(permissionGranted: Boolean, permanentlyDenied: Boolean, onRequest: () -> Unit) {
    ...
    if (!permissionGranted) {
        // User-instructed: this banner is nested inside a ScanEatCard in each
        // tracker card - matches ReminderRow.kt's own sub-card standard for
        // that exact role (clip+background+padding, no shadow/border/custom
        // accent tint) instead of a separately-styled coral Surface.
        Column(
