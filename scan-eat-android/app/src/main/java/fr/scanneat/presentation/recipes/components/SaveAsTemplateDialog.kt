...
/**
 * A Recipe has no meal of its own (it collapses into one diary entry, unlike a
 * template's per-item meal), so converting it into a Saved Meal template needs
 * to ask which slot the result should apply to. Same meal-slot chip pattern
 * LogRecipeDialog already uses.
 */
@Composable
internal fun SaveAsTemplateDialog(recipe: Recipe, onDismiss: () -> Unit, onConfirm: (MealSlot) -> Unit) {
    ...
    AlertDialog(
        ...
        confirmButton = { TextButton(onClick = { onConfirm(slot) }) { Text(stringResource(R.string.common_save), color = AccentCoral) } },
