...
private fun ScoreChip(label: String, color: Color) {
    Surface(shape = RoundedCornerShape(CardRadius.CONTROL), color = color.copy(alpha = 0.15f), shadowElevation = 0.dp, modifier = Modifier.shadow(elevation = 4.dp, shape = RoundedCornerShape(CardRadius.CONTROL)).clip(RoundedCornerShape(CardRadius.CONTROL))) {
        Text(
            label,
